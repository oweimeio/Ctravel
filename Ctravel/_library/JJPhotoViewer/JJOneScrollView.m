...
@interface JJOneScrollView()<UIScrollViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate>
{
...
-(void)setNetWorkImage:(UIImageView *)imageView urlStr:(NSString *)urlStr
{
	...
	[UIView animateWithDuration:AnimationTime animations:^{
		...
		self.userInteractionEnabled = YES ;
		//变换完动画 从网络开始加载图
		[self.mainImageView setImageWithURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlStr]] placeholderImage:self.mainImageView.image success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
			//下载成功
			[self addGestureRecognizer:self.twoTap];
...
-(void)setFrameAndZoom:(UIImageView *)imageView
{
	...
	if(imageView.image == nil || imageView.image.size.width == 0 || imageView.image.size.height ==0)
	{
		...
		imageH = mainH;
		imageW = mainW;
		self.mainImageView.image = [UIImage imageNamed:@"placeholder-none"];
...
-(void)actionSheet:(UIActionSheet*)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex {
	if([actionSheet.title containsString:@"保存图片"]) {
		if(buttonIndex ==0) {
			
			UIImageWriteToSavedPhotosAlbum(_mainImageView.image,self,@selector(imageSavedToPhotosAlbum:didFinishSavingWithError:contextInfo:),nil);
		}
	}
}
